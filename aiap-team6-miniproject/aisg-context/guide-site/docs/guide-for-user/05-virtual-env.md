...
- First, have VSCode open the repository that you have cloned
  ...
  `/polyaxon-v1-data/workspaces/<YOUR_NAME>/aiap-team6-miniproject`.

- Now, let's initialise `conda` for the bash shell, and create
  the virtual environment specified in
  `aiap-team6-miniproject-conda-env.yml`.
...
$ source ~/.bashrc
(base) $ conda env create -f aiap-team6-miniproject-conda-env.yml \
            -p /polyaxon-v1-data/workspaces/<YOUR_NAME>/conda_envs/aiap-team6-miniproject-conda-env
```
...
=== "Polyaxon VSCode Terminal"
    ...
    ```bash
    (base) $ echo 'alias aiap-team6-miniproject-conda-env="conda activate /polyaxon-v1-data/workspaces/<YOUR_NAME>/conda_envs/aiap-team6-miniproject-conda-env"' >> ~/.bashrc
    (base) $ source ~/.bashrc
    (base) $ aiap-team6-miniproject-conda-env
    (aiap-team6-miniproject-conda-env) $ # conda environment has been activated
    ```
...
- Ensure that you have
  ...
  `aiap-team6-miniproject-conda-env.yml`. You can check for the
  library like so:

=== "Polyaxon VSCode Terminal"

    ```bash
    $ conda activate /polyaxon-v1-data/workspaces/<YOUR_NAME>/conda_envs/aiap-team6-miniproject-conda-env
...
=== "JupyterLab Terminal"
    ...
    ```bash
    $ ipython kernel install --name "aiap-team6-miniproject-conda-env" --user
    ```
