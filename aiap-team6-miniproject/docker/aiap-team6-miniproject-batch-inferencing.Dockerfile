...
ARG REPO_DIR="."
ARG CONDA_ENV_FILE="aiap-team6-miniproject-conda-env.yml"
ARG CONDA_ENV_NAME="aiap-team6-miniproject"
...
ARG PRED_MODEL_GCS_URI="gs://aiap-team6-miniproject-artifacts/mlflow-tracking-server/team6_fl_msd/$PRED_MODEL_UUID"
...
COPY $REPO_DIR aiap-team6-miniproject
...
RUN $CONDA_BIN env create -f aiap-team6-miniproject/$CONDA_ENV_FILE && \
    $CONDA_BIN init bash && \
    $CONDA_BIN clean -a -y && \
    echo "source activate $CONDA_ENV_NAME" >> "$HOME_DIR/.bashrc"

WORKDIR $HOME_DIR/aiap-team6-miniproject
