...
`docker/aiap-team6-miniproject-poly-vscode.Dockerfile` to build a
...
=== "Linux/macOS"
    ...
    $ docker build \
        -t asia.gcr.io/$GCP_PROJECT_ID/team6_fl_msd/vscode-server:0.1.0 \
        -f docker/aiap-team6-miniproject-poly-vscode.Dockerfile \
        --platform linux/amd64 .
    $ docker push asia.gcr.io/$GCP_PROJECT_ID/team6_fl_msd/vscode-server:0.1.0
    ```
...
=== "Linux/macOS"
    ...
    $ polyaxon run -f aisg-context/polyaxon/polyaxonfiles/vscode-service.yml \
        -P DOCKER_IMAGE="asia.gcr.io/$GCP_PROJECT_ID/team6_fl_msd/vscode-server:0.1.0" \
...
`http://localhost:8117/ui/default/aiap-team6-miniproject-<YOUR_NAME>/services`.
...
=== "Linux/macOS"
    ...
    $ export GCP_PROJECT_ID=aiap-10-ds
    $ docker build \
        -t asia.gcr.io/$GCP_PROJECT_ID/team6_fl_msd/jupyter-server:0.1.0 \
        -f docker/aiap-team6-miniproject-poly-jupyter.Dockerfile \
        --platform linux/amd64 .
    $ docker push asia.gcr.io/$GCP_PROJECT_ID/team6_fl_msd/jupyter-server:0.1.0
    ```
...
=== "Linux/macOS"
    ...
    $ polyaxon run -f aisg-context/polyaxon/polyaxonfiles/jupyter-service.yml \
        -P DOCKER_IMAGE="asia.gcr.io/$GCP_PROJECT_ID/team6_fl_msd/jupyter-server:0.1.0" \
        -P WORKING_DIR="/polyaxon-v1-data" \
        -p aiap-team6-miniproject-<YOUR_NAME>
    ```
...
```bash
$ gcloud auth activate-service-account <SA_CLIENT_EMAIL_FROM_SA_KEY>@aiap-10-ds.iam.gserviceaccount.com --key-file=$GOOGLE_APPLICATION_CREDENTIALS
```
